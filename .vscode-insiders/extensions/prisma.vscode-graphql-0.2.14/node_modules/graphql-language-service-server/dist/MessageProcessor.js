...
class MessageProcessor {
  constructor(logger, watchmanClient) {
  ...
  async handleInitializeRequest(params, token, configDir) {
    if (!params) {
    ...
    const rootPath = (0, _path.dirname)((0, _graphqlConfig.findGraphQLConfigFile)(configDir ? configDir.trim() : params.rootPath));

    if (!rootPath) {
      throw new Error('`--configDir` option or `rootPath` argument is required.');
    }

    this._graphQLCache = await (0, _GraphQLCache.getGraphQLCache)(rootPath);
    const config = (0, _graphqlConfig.getGraphQLConfig)(rootPath);
  ...
  async _subcribeWatchman(config, watchmanClient) {
    if (!watchmanClient) {
      return;
    }
    ...
    try {
      ...
      await watchmanClient.checkVersion(); // Otherwise, subcribe watchman according to project config(s).

      const projectMap = config.getProjects();
      let projectConfigs = projectMap ? Object.values(projectMap) : []; // There can either be a single config or one or more project
      ...
      projectConfigs.forEach(projectConfig => {
        watchmanClient.subscribe(projectConfig.configDir, this._graphQLCache.handleWatchmanSubscribeEvent(config.configDir, projectConfig));
      });
    } catch (err) {
  ...
  async handleDidOpenOrSaveNotification(params) {
    ...
    const diagnostics = [];
    let contents = []; // Create/modify the cached entry if text is provided.
    ...
    await Promise.all(contents.map(async ({
      query,
      range
    }) => {
  ...
  async handleDidChangeNotification(params) {
    ...
    const contentChange = contentChanges[contentChanges.length - 1]; // As `contentChanges` is an array and we just want the
    // latest update to the text, grab the last entry from the array.

    const uri = textDocument.uri || params.uri; // If it's a .js file, try parsing the contents to see if GraphQL queries
    // exist. If not found, delete from the cache.

    const contents = getQueryAndRange(contentChange.text, uri); // If it's a .graphql file, proceed normally and invalidate the cache.
  ...
  async handleCompletionRequest(params, token) {
    if (!this._isInitialized) {
      // $FlowFixMe
      return [];
    }
    ...
    const textDocument = params.textDocument;
    const position = params.position; // `textDocument/completion` event takes advantage of the fact that
  ...
  async handleHoverRequest(params, token) {
    if (!this._isInitialized) {
      return [];
    }
  ...
  async handleWatchedFilesChangedNotification(params) {
    ...
    return Promise.all(params.changes.map(async change => {
      if (change.type === _graphqlLanguageServiceTypes.FileChangeTypeKind.Created || change.type === _graphqlLanguageServiceTypes.FileChangeTypeKind.Changed) {
        ...
        const text = (0, _fs.readFileSync)(new _url.URL(uri).pathname).toString();
        ...
        const diagnostics = (await Promise.all(contents.map(async ({
          query,
          range
        }) => {
          const results = await this._languageService.getDiagnostics(query, uri);
      ...
      } else if (change.type === _graphqlLanguageServiceTypes.FileChangeTypeKind.Deleted) {
        this._graphQLCache.updateFragmentDefinitionCache(this._graphQLCache.getGraphQLConfig().configDir, change.uri, false);

        this._graphQLCache.updateObjectTypeDefinitionCache(this._graphQLCache.getGraphQLConfig().configDir, change.uri, false);
      }
    }));
  }

  async handleDefinitionRequest(params, token) {
    if (!this._isInitialized) {
      return [];
    }
  ...
  async _updateFragmentDefinition(uri, contents) {
    ...
    await this._graphQLCache.updateFragmentDefinition(rootDir, new _url.URL(uri).pathname, contents);
  }

  async _updateObjectTypeDefinition(uri, contents) {
    const rootDir = this._graphQLCache.getGraphQLConfig().configDir;

    await this._graphQLCache.updateObjectTypeDefinition(rootDir, new _url.URL(uri).pathname, contents);
  }
...
function getQueryAndRange(text, uri) {
  ...
  if ((0, _path.extname)(uri) === '.js') {
    ...
    const templates = (0, _findGraphQLTags.findGraphQLTags)(text);
    ...
    const range = new _graphqlLanguageServiceUtils.Range(new _graphqlLanguageServiceUtils.Position(0, 0), new _graphqlLanguageServiceUtils.Position(lines.length - 1, lines[lines.length - 1].length - 1));
    return [{
      query,
      range
    }];
...
function processDiagnosticsMessage(results, query, range) {
  ...
  const lastCharacterPosition = new _graphqlLanguageServiceUtils.Position(totalLines, lastLineLength);
  const processedResults = results.filter(diagnostic => diagnostic.range.end.lessThanOrEqualTo(lastCharacterPosition));

  if (range) {
    const offset = range.start;
    return processedResults.map(diagnostic => ({ ...diagnostic,
