...
exports.tmpDir = tmp.dirSync({ prefix: 'queries_', keep: false, unsafeCleanup: true });
exports.upgradesTmpDir = tmp.dirSync({ dir: exports.tmpDir.name, prefix: 'upgrades_', keep: false, unsafeCleanup: true });
exports.tmpDirDisposal = {
    dispose: () => {
        exports.upgradesTmpDir.removeCallback();
        exports.tmpDir.removeCallback();
    }
};
class UserCancellationException extends Error {
}
exports.UserCancellationException = UserCancellationException;
/**
 ...
 */
class QueryInfo {
    constructor(program, dbItem, queryDbscheme, // the dbscheme file the query expects, based on library path resolution
    quickEvalPosition, metadata, templates) {
        this.program = program;
        this.dbItem = dbItem;
        this.queryDbscheme = queryDbscheme;
        this.quickEvalPosition = quickEvalPosition;
        this.metadata = metadata;
        this.templates = templates;
        this.queryID = QueryInfo.nextQueryId++;
        this.compiledQueryPath = path.join(exports.tmpDir.name, `compiledQuery${this.queryID}.qlo`);
        this.resultsPaths = {
            resultsPath: path.join(exports.tmpDir.name, `results${this.queryID}.bqrs`),
            interpretedResultsPath: path.join(exports.tmpDir.name, `interpretedResults${this.queryID}.sarif`),
        };
    ...
    async compile(qs) {
        ...
        return ((compiled === null || compiled === void 0 ? void 0 : compiled.messages) || []).filter(msg => msg.severity === messages.Severity.ERROR);
    }
    /**
     * Holds if this query can in principle produce interpreted results.
     */
    async canHaveInterpretedResults() {
        const hasMetadataFile = await this.dbItem.hasMetadataFile();
        if (!hasMetadataFile) {
            logging_1.logger.log("Cannot produce interpreted results since the database does not have a .dbinfo or codeql-database.yml file.");
        }
     ...
     */
    async hasInterpretedResults() {
        return fs.pathExists(this.resultsPaths.interpretedResultsPath);
    }
}
exports.QueryInfo = QueryInfo;
QueryInfo.nextQueryId = 0;
async function clearCacheInDatabase(qs, dbItem) {
    if (dbItem.contents === undefined) {
    ...
    return helpers.withProgress({
        ...
        cancellable: false,
    }, (progress, token) => qs.sendRequest(messages.clearCache, params, token, progress));
}
exports.clearCacheInDatabase = clearCacheInDatabase;
/**
 *
 * @param filePath This needs to be equivalent to java Path.toRealPath(NO_FOLLOW_LINKS)
 *
 */
async function convertToQlPath(filePath) {
    if (process.platform === "win32") {
        if (path.parse(filePath).root === filePath) {
            ...
            const fileNames = await util_1.promisify(fs.readdir)(dir);
...
async function getSelectedPosition(editor) {
 ...
 */
async function checkDbschemeCompatibility(cliServer, qs, query) {
    ...
    if (query.dbItem.contents !== undefined && query.dbItem.contents.dbSchemeUri !== undefined) {
        ...
        const hash = async function (filename) {
        ...
        if (upgradableTo != dbschemeOfLib) {
            logging_1.logger.log(`Query ${query.program.queryPath} expects database scheme ${query.queryDbscheme}, but database has scheme ${query.program.dbschemePath}, and no upgrade path found`);
        ...
        if (upgradableTo == dbschemeOfLib &&
            ...
            await upgrades_1.upgradeDatabase(qs, query.dbItem, vscode.Uri.file(finalDbscheme), databases_1.getUpgradesDirectories(scripts));
        }
    }
}
/**
 ...
 */
async function promptUserToSaveChanges(document) {
    if (document.isDirty) {
...
*/
async function determineSelectedQuery(selectedResourceUri, quickEval) {
    ...
    let quickEvalPosition = undefined;
    let quickEvalText = undefined;
    if (quickEval) {
        if (editor == undefined) {
...
exports.determineSelectedQuery = determineSelectedQuery;
async function compileAndRunQueryAgainstDatabase(cliServer, qs, db, quickEval, selectedQueryUri, templates) {
    ...
    historyItemOptions.isQuickQuery === quick_query_1.isQuickQueryPath(queryPath);
    if (quickEval) {
    ...
    if (querySchemaName != dbSchemaName) {
        logging_1.logger.log(`Query schema was ${querySchemaName}, but database schema was ${dbSchemaName}.`);
    ...
    try {
        ...
        logging_1.logger.log(`Couldn't resolve metadata for ${qlProgram.queryPath}: ${e}`);
    }
    ...
    try {
        ...
        if (e instanceof vscode_languageclient_1.ResponseError && e.code == vscode_languageclient_1.ErrorCodes.RequestCancelled) {
...
exports.compileAndRunQueryAgainstDatabase = compileAndRunQueryAgainstDatabase;
function createSyntheticResult(query, db, historyItemOptions, message, resultType) {
    return {
        query,
        result: {
            evaluationTime: 0,
...
//# sourceMappingURL=run-queries.js.map
