...
- Reviewer findings are advice, not authorization: do not implement speculative hardening, architecture changes, or scope expansion merely to satisfy a reviewer; reject or defer findings that conflict with explicit user requirements, established behavior, or the requested scope, and fix only concrete issues demonstrated to affect the task.
